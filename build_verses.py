...
DARK = re.compile(
    ...
    # Apocalyptic and dated-prophecy language. 스가랴 14:7 ("여호와께서 아시는 한 날이 있으리니
    # 낮도 아니요 밤도 아니라") passed the lexicon as 위로 and was chosen for a morning post: it is
    # genuine 개역개정 and completely wrong for a daily verse.
    r"그 날에|그 때에|여호와의 날|말일에|마지막 날|나팔|짐승이|용이|재림|천사가|인을 떼|"
    r"해가 어두워|달이 빛을|별들이|하늘이 떠나|땅이 진동|지진")
...
- 아침에 읽기에 어둡거나 위협적인 것
- 종말·재난·예언의 날에 관한 것("그 날에", "여호와의 날" 같은 예언 문맥)
- 1인칭 탄식·호소("나는 가난하고 슬프오니" 같은 것)
- 예언서·묵시의 특수한 배경을 알아야만 뜻이 통하는 것

판정 기준은 하나입니다: **오늘 아침 이 계정을 처음 보는 사람이, 배경 설명 없이 읽고
바로 이해하고 힘을 얻는가.** 조금이라도 망설여지면 거절하세요. 후보는 충분히 많습니다.
