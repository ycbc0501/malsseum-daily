...
# ── Seasonal greeting: CAPTION TEXT ONLY ─────────────────────────────────────────────────
# Nothing here ever reaches the picture. LAW rule 1 forbids writing in frame and the overlay
# still carries the 말씀 alone — this is the line Instagram shows, not the line the video shows.
# check_rules B5b asserts the string appears in no prompt or render module.
#
# Keyed by explicit KST date. 추석/설날 move on the lunar calendar and this repo has no lunar
# library, so a table someone must write per year is the honest form — and an unlisted date gets
# nothing, which means the greeting EXPIRES BY ITSELF instead of quietly wishing a happy 추석 in
# March. Entries are kept after the fact as a record of what shipped.
GREETINGS = {
    "2026-09-24": "풍성한 한가위 보내세요.",   # 연휴 첫날 (저녁 슬롯부터)
    "2026-09-25": "풍성한 한가위 보내세요.",   # 추석 당일
    "2026-09-26": "풍성한 한가위 보내세요.",   # 연휴, 마지막 게시일
}


def greeting(today=None):
    """The greeting for this KST date, or None.

    A pure function of the date so tests can pass one in rather than wait a year for it. 9-27
    (연휴 마지막 날, 일요일 캐러셀) is deliberately absent: by then the wish has passed.
    """
    day = today or datetime.now(KST).strftime("%Y-%m-%d")
    return GREETINGS.get(day)


def build_caption(verse, translation, today=None):
    ...
    SECOND EXCEPTION, date-gated (2026-09-24): a 명절 greeting, rule B-5b. It goes directly under
    the reference and ABOVE the reflection line, because Instagram folds the caption at ~125 chars
    behind "더 보기" and the 08-01 measurement found that cut landing right after the reference.
    Put below the reflection it would ship folded away, which is how the CTA disappeared."""
    body = f"{verse['text']}\n[{verse['ref']}]"
    hello = greeting(today)
    if hello:                             # under the 출처, above our line — never above the 말씀
        body = f"{body}\n\n{hello}"
    try:
        import reflection
